...
const STORAGE_KEY = 'SATM_SECURE_VAULT_V6';
...
const App: React.FC = () => {
  ...
  const handleFinalSubmit = () => {
    if (!responses[46]) return alert("Please authorize the research protocol.");
    ...
    // Required for multi-page Google Forms to register all entries
  ...
  if (status === 'complete') {
    return (
      <div className="min-h-screen flex items-center justify-center p-6 bg-[#020617]">
        <div className="max-w-2xl w-full p-16 bg-slate-900/40 border border-white/10 rounded-[4rem] text-center backdrop-blur-3xl shadow-2xl">
          <h2 className="text-7xl serif italic text-white mb-6">Archived.</h2>
          <p className="text-slate-400 text-2xl font-light mb-12">Transmission successful. The <span className="text-indigo-400 font-bold">@ST-research-team</span> thanks you.</p>
          <button onClick={() => window.location.reload()} className="px-16 py-7 bg-white text-black rounded-full font-black text-xs uppercase tracking-[0.4em] hover:scale-105 transition-all">Start Over</button>
        </div>
      </div>
    );
  }
  ...
  if (step === -1) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-8 bg-[#020617] text-center">
        <div className="text-indigo-500 font-black text-[10px] uppercase tracking-[1.5em] mb-12 animate-pulse">Self-Authentication Theory of Motivation</div>
        <h1 className="text-[15vw] lg:text-[14rem] serif font-bold italic text-white leading-[0.8] mb-12 select-none tracking-tighter">
          SATM <span className="text-indigo-600 not-italic opacity-80">PROTOCOL</span>
        </h1>
        <p className="max-w-2xl text-slate-400 text-2xl lg:text-3xl font-light leading-relaxed mb-20">
          A research exploration by the <span className="text-white font-semibold">@ST-research-team</span> into authenticity and motivation.
        </p>
        <button 
          onClick={() => setStep(0)} 
          className="px-28 py-10 bg-white text-black rounded-full font-black text-3xl hover:scale-110 active:scale-95 transition-all shadow-[0_30px_100px_rgba(255,255,255,0.1)]"
        >
          Begin Archive
        </button>
        <div className="mt-24 text-[11px] font-black uppercase tracking-[1em] text-slate-800">@ST-research-team</div>
      </div>
    );
  }

  const q = QUESTIONS[step];
  if (!q) return <div className="bg-[#020617] min-h-screen text-white p-20">Loading Protocol...</div>;
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#020617] text-slate-100 selection:bg-indigo-500/30">
      ...
      {/* Persistent Header */}
      <div className="sticky top-0 w-full z-50 p-6 lg:p-10 flex justify-between items-center bg-[#020617]/95 backdrop-blur-xl border-b border-white/5">
        <div className="flex items-center gap-6">
          <div className="w-[4px] h-10 bg-indigo-500 rounded-full shadow-[0_0_15px_rgba(99,102,241,0.5)]"></div>
          <div>
            <div className="text-[10px] font-black uppercase tracking-widest text-slate-500">Entry Sequence {q.id}</div>
            <div className="text-xl lg:text-2xl font-bold text-indigo-400 uppercase tracking-[0.2em]">{q.section}</div>
          </div>
        </div>
        <div className="hidden lg:block">
           <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest text-right mb-2">{Math.round(progress)}% Progress</div>
      ...
      {/* Main Container */}
      <div className="flex-1 flex flex-col lg:flex-row w-full max-w-[1700px] mx-auto min-h-0">
        
        {/* Left Side: Question (Readability prioritized) */}
        <div className="flex-1 p-10 lg:p-24 lg:pt-32 flex flex-col">
          <div className="fade-in" key={q.id}>
            <h2 className="text-4xl lg:text-7xl serif italic text-white leading-[1.15] tracking-tight mb-8">
              {q.text}
            </h2>
        ...
        {/* Right Side: Inputs */}
        <div className="flex-1 p-6 lg:p-20 lg:pt-32 bg-slate-900/20 backdrop-blur-sm lg:border-l border-white/5 shadow-[-50px_0_100px_rgba(0,0,0,0.5)]">
          <div className="max-w-xl mx-auto w-full fade-in" key={`input-${q.id}`}>
            
            <div className="min-h-[400px] flex flex-col justify-start py-8">
              {q.type === 'choice' && (
                <div className="space-y-4">
                  {q.options?.map((opt) => (
                    <button 
                      ...
                      className={`w-full p-8 text-left rounded-[2.5rem] border-2 transition-all flex justify-between items-center group active:scale-[0.98] ${responses[q.id] === opt ? 'border-indigo-500 bg-indigo-500/10' : 'border-white/5 bg-white/5 hover:border-white/10'}`}
                    >
                  ...
                  <div className="flex justify-between gap-3 mb-16">
                    {[1, 2, 3, 4, 5].map(num => (
                      <button 
                        ...
                        className={`flex-1 aspect-square rounded-3xl text-4xl lg:text-5xl font-black border-2 transition-all duration-300 flex items-center justify-center active:scale-75 ${responses[q.id] === num ? 'bg-indigo-600 border-indigo-600 text-white scale-[1.3] shadow-[0_25px_60px_rgba(79,70,229,0.5)] z-10' : 'border-white/5 bg-white/5 text-slate-800 hover:text-slate-300 hover:scale-105'}`}
                      >
                        {num}
                      </button>
                    ))}
              ...
              {q.type === 'consent' && (
                <button 
                  ...
                  className={`w-full p-12 rounded-[4rem] border-2 text-left flex items-center gap-10 transition-all ${responses[q.id] ? 'border-indigo-500 bg-indigo-500/10 shadow-2xl' : 'border-white/10 bg-white/5'}`}
                >
            ...
            {/* Navigation */}
            <div className="mt-12 pt-12 border-t border-white/5 flex items-center justify-between pb-24 lg:pb-0">
              <button 
                onClick={() => setStep(s => s - 1)} 
                disabled={step === 0}
                className="text-xs font-black uppercase tracking-widest text-slate-700 hover:text-white disabled:opacity-0 transition-all py-4"
              >
                Back
              </button>
              ...
              {step < QUESTIONS.length - 1 ? (
                <button 
                  onClick={() => setStep(s => s + 1)}
                  className="px-20 py-8 bg-white text-black rounded-full font-black text-xs uppercase tracking-[0.4em] hover:scale-110 transition-all shadow-xl"
                >
                  Proceed
                </button>
              ) : (
                <button 
                  ...
                  className="px-20 py-8 bg-indigo-600 text-white rounded-full font-black text-xs uppercase tracking-[0.4em] hover:scale-110 transition-all shadow-xl shadow-indigo-600/30 disabled:opacity-50"
                >
            ...
            <div className="mt-12 text-center text-[11px] font-black uppercase tracking-[1em] text-slate-800/60 py-4">
              @ST-research-team
            </div>
          </div>
        </div>
